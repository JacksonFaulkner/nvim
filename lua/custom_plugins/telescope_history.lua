-- Smart history for telescope, based on nvim-telescope/telescope-smart-history.nvim
-- Requires sqlite.lua (kkharji/sqlite.lua)
...
local get_smart_history = function()
  ...
  if not has_sqlite then
    if type(sqlite) ~= "string" then
      print "Couldn't find sqlite.lua. Using simple history"
    else
...
function M.setup()
  ...
  local config = telescope.setup and require("telescope.config").values or {}
